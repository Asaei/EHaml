<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="View.ascx.cs" Inherits="EHamlDashboard.View" %>
...
//        }
        ...
        $.ajax({
            type: "GET",
            url: sf0.getServiceRoot('EHamlLibrary') + "Dashboard/GetSelectedGridForDashboardeSahebBar",
    ...
    function bindGrdS(type) {
        if (type == 'استعلامات فعال') {
            ...
            $.ajax({
                type: "GET",
                url: sf2.getServiceRoot('EHamlLibrary') + "Dashboard/GetMyInquiryList",
            ...
            $.ajax({
                type: "GET",
                url: sf00.getServiceRoot('EHamlLibrary') + "Dashboard/GetMyInquiryList",
    ...
    function bindMyInquiryReplyListStep2S(myInquirysReply) {
        $("#grdMyInquirysReplyd").kendoGrid({
            ...
            },
            {
                ...
                width: "14px",
                template:
                    function (dataItem) { return "<span class='JoziyatePishnahad'><a href='javascript:void(0)' class='k-button k-button-icontext' onclick='JoziyatePishnahadeBeEstelamEstelamS(" + dataItem.ReplyToInquiryId + ',' + 0 + ")'>جزئیات پیشنهاد</a></span>" }
    },
                {
                    title: "عملیات",
    ...
    function JoziyatePishnahadeBeEstelamEstelamS(replyToInquiryId, inquiryType) {
        var modulePath = "/default.aspx?tabid=5175&ReplyToInquiryId=" + replyToInquiryId + "&InquiryType=Zadghan" + "&popUp=true";
        dnnModal.show(modulePath, true, 550, 960, false, '');
    }

    function JoziyateEstelamS(link) {
        var modulePath = link + '&popUp=true';
